...
class PPOSaveSameModelCB(CheckpointCallback):
    ...
    def _save_overwrite(self):
        ...
        # ✅ Only save replay buffer if it exists (for off-policy algorithms like DDPG, SAC, etc.)
        if self.save_replay_buffer and hasattr(self.model, "replay_buffer") and self.model.replay_buffer is not None:
            self.model.save_replay_buffer(self.replay_buffer_path)

        # ✅ Only save vecnormalize if the method exists and returns a valid object
